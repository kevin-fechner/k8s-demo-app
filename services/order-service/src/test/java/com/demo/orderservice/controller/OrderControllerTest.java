...
class OrderControllerTest {
  ...
  void setUp() {
    ...
    testRequest =
        new OrderRequest(
            "John Doe", "john@example.com", List.of(new OrderItemRequest(1L, 2)), null);
  }
  ...
  void getOrders_WithCursor_Returns200() throws Exception {
    ...
    mockMvc
        .perform(get("/api/orders").param("cursor", cursor))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.data[0].customerName").value("John Doe"));
  }
  ...
  void getOrders_SizeExceeds100_CappedAt100() throws Exception {
    ...
    mockMvc.perform(get("/api/orders").param("size", "200")).andExpect(status().isOk());
  ...
  void getOrders_WithStatusFilter_PassesFilterToService() throws Exception {
    ...
    mockMvc.perform(get("/api/orders").param("status", "PENDING")).andExpect(status().isOk());

    verify(orderService)
        .getOrders(isNull(), eq(10), eq(new OrderFilter(OrderStatus.PENDING, null, null, null)));
  }
  ...
  void createOrder_ProductNotAvailable_Returns422() throws Exception {
    when(orderService.createOrder(any())).thenThrow(new ProductNotAvailableException(1L));
  ...
  void createOrder_InvalidRequest_Returns400() throws Exception {
    OrderRequest invalid =
        new OrderRequest(null, "john@example.com", List.of(new OrderItemRequest(1L, 2)), null);
  ...
  void deleteOrder_Returns404() throws Exception {
    ...
    mockMvc.perform(delete("/api/orders/99")).andExpect(status().isNotFound());
  }

  @Test
  @DisplayName("PATCH /api/orders/{id}/status - should return 404 when not found")
  void updateStatus_Returns404() throws Exception {
    when(orderService.updateStatus(eq(99L), any())).thenThrow(new OrderNotFoundException(99L));
