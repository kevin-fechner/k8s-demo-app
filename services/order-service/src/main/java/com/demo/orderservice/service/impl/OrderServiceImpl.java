...
public class OrderServiceImpl implements OrderService {
  ...
  public OrderServiceImpl(
    ...
    this.ordersConfirmedCounter =
        Counter.builder("business.orders.confirmed")
            .description("Total number of orders confirmed")
            .register(meterRegistry);
    this.ordersCancelledCounter =
  ...
  @Override
  @Transactional(readOnly = true)
  public CursorPage<OrderResponse> getOrders(String cursor, int size, OrderFilter filter) {

    Long cursorId = CursorUtil.decode(cursor);
    Pageable pageable = PageRequest.of(0, size + 1);

    List<Order> orders =
        orderRepository.findWithCursor(
            cursorId,
            filter.status(),
    ...
    String nextCursor = hasMore ? CursorUtil.encode(pageData.getLast().getId()) : null;
    ...
    for (OrderItemRequest itemRequest : request.items()) {
      ProductDto product =
          productClient
              ...
              .orElseThrow(() -> new ProductNotAvailableException(itemRequest.productId()));
